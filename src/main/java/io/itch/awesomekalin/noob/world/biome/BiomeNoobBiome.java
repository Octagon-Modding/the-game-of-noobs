...
import io.itch.awesomekalin.noob.block.BlockNoobLeaves;
import io.itch.awesomekalin.noob.block.BlockNoobChest;
import io.itch.awesomekalin.noob.ElementsNoobMod;

@ElementsNoobMod.ModElement.Tag
public class BiomeNoobBiome extends ElementsNoobMod.ModElement {
	@GameRegistry.ObjectHolder("noob:noob_biome")
	public static final BiomeGenCustom biome = null;
	public BiomeNoobBiome(ElementsNoobMod instance) {
		super(instance, 167);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Noob Biome").setRainfall(1F).setBaseHeight(0.1F).setWaterColor(-52429).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("noob_biome");
			topBlock = Blocks.GRASS.getDefaultState();
			fillerBlock = BlockNoobChest.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.GRASS.getDefaultState().getBlock() || ground == BlockNoobChest.block.getDefaultState().getBlock())
							&& (ground2 == Blocks.GRASS.getDefaultState().getBlock()
									|| ground2 == BlockNoobChest.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockNoobChest.block.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockNoobLeaves.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockNoobLeaves.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockNoobLeaves.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockNoobLog.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockNoobLog.block.getDefaultState().getBlock()
					|| blockType == BlockNoobLeaves.block.getDefaultState().getBlock() || blockType == Blocks.GRASS.getDefaultState().getBlock()
					|| blockType == BlockNoobChest.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockNoobChest.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockNoobChest.block.getDefaultState());
		}
